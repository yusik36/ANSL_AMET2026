...
## 대회 트랙 실측 (`71e69ee938032295503bfed557fde18c`, "AMET 2026")

`tools/YS_track_dump.py`로 시뮬레이터 `/sim/api/world`에서 뽑은 값 (2026-08-19).
번들에는 이 트랙이 없다 — 번들이 8/11자라 그 안에는 연습 트랙(35.56 m)뿐이다.

```
길이            30.505 m       점 388개 (간격 0.0786 m)
경계상자        x 1.38..10.90   y 1.21..5.82
출발점          (1.400, 3.395)  방위 -1.5707 rad (-90.0deg)
```
...
| | 전체 폭 | 안쪽 반폭 | 바깥 반폭 |
|---|---|---|---|
| min | **0.700** | 0.350 | 0.346 |
| mean | 0.724 | 0.364 | 0.361 |
| max | 0.875 | 0.517 | 0.497 |

백분위: 1~25%가 전부 0.700, 50%가 0.704, 75%가 0.728, 95%가 0.827.
**평균 0.724는 소수의 넓은 코너 바깥쪽이 끌어올린 값이고, 트랙의 절반 이상이 정확히
0.700 m다.** 한 바퀴를 12구간으로 나눠도 각 구간의 최솟값은 전부 0.700~0.701 —
좁아지는 병목은 없고 넓어지는 보너스만 있다.

차폭 0.16 m 기준: 가장 좁은 곳에서 좌우 합쳐 0.540 m 여유, **중심선 기준 편측 0.270 m.**
이게 코너 커팅에 쓸 수 있는 전부다. 차보다 좁은 지점은 0개.

가장 좁은 구간은 s ≈ 20.1~20.5 m 한 곳에 몰려 있다 (x 7.6~8.0, y 3.76).

### 곡률 — 중심선은 어떤 스케일에서도 못 돈다

| 스텐실 | 최소반경 | 필요 조향 | 가능? | 위치 |
|---|---|---|---|---|
| 0.18 m | 0.068 m | 69.23° | ✕ | s=28.4 m |
| 0.25 m | 0.097 m | 61.57° | ✕ | s=28.4 m |
| 0.37 m | 0.170 m | 46.69° | ✕ | s=19.8 m |
| 0.50 m | 0.205 m | 41.34° | ✕ | s=19.8 m |
| 0.75 m | 0.431 m | 22.66° | ✕ | s=19.8 m |

차 한계는 조향 20° → 최소반경 0.495 m. 가장 관대한 스텐실에서도 22.66°가 필요하다.
**코너 커팅은 최적화가 아니라 완주의 전제조건이다** — 그리고 편측 여유 0.270 m가 그
커팅 예산의 전부다. 최적화된 레이싱라인은 최소반경 0.581 m / 조향 17.2°로 통과한다.

병목 두 곳(s≈19.8 m, s≈28.4 m)이 폭 최소 구간(s≈20 m)과 겹치는 곳이 가장 어렵다.

### 오브젝트
`cone1`~`cone6`, `light1`, `wall_0`~`wall_3` (11개). `/world`가 이름만 주고 좌표는 안 준다.
**본선에서는 콘 위치만 바뀌므로 그때 다시 뽑을 것.**

> 원본 기하는 `YS_track_dump.py --save`로 `.npz`에 저장할 수 있다 (centre/inner/outer/
> cum_s/width). 시뮬레이터가 떠 있어야 뽑히므로, 대회 당일 트랙이 갱신되면 가장 먼저
> 이걸 돌릴 것.
